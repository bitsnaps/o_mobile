...
public class IrModel extends OModel {
    ...
    public void initDaos() {
        ...
        syncDao =  daoRepo.getDao(IrModel.class);
    }
    @Override
    ...
    private SyncModel create(String modelName, SyncModel parent, Columns.SyncModel.Mode syncMode, Integer syncDepth) {
        ...
        if(syncModel.getSyncDepth() > 0){
           OModel syncModelDao = (OModel) daoRepo.getDao(modelName);
            List<OColumn> childColumns = syncModelDao.getRelationColumns();
            for(OColumn childModelColumn: childColumns){
                ...
                String childModelName = ((OModel) daoRepo.getDao((Class<? extends OModel>)childModelColumn.getType())).getModelName();
