...
import com.odoo.odoorx.core.base.rpc.helper.OdooVersion;
import com.odoo.odoorx.core.base.rpc.listeners.IModuleInstallListener;
import com.odoo.odoorx.core.base.service.ISyncServiceListener;
...
import com.odoo.odoorx.core.base.utils.OCursorUtils;
import com.odoo.odoorx.core.base.utils.ODateUtils;
import com.odoo.odoorx.core.base.utils.OListUtils;
import com.odoo.odoorx.core.base.utils.OStorageUtils;
...
public class OModel implements ISyncServiceListener {
    ...
    private String BASE_AUTHORITY = BuildConfig.APPLICATION_ID + ".base.provider.content";
                ...
                for (Annotation annotation : annotations) {
                    ...
                    if (type.getDeclaringClass().isAssignableFrom(Odoo.api.class)) {
                        switch (mOdooVersion.getVersionNumber()) {
                            case 12:
                                if (type.isAssignableFrom(Odoo.api.v12.class)) {
                                    version++;
                                }
                                break;
                            case 11:
    ...
    public static OModel get(Context context, String model_name, String username) {
        ...
        return DaoRepoBase.getInstance().getModel(context, model_name, user);
    }
    ...
    public String getLastSyncDateTime() {
        ...
        List<ODataRow> records = model.select(null, "model_name = ?", new String[]{getModelName()});
        if (records.size() > 0) {
    ...
    private void handleOneToManyRecords(OColumn column, RelCommands commands, OModel relModel,
        ...
        if (commands == RelCommands.Replace) {
            ...
            int count = relModel.update(column.getRelatedColumn() + " = ?", new String[]{record_id + ""},
                    old_values);
            Log.i(TAG, String.format("#%id references removed " + relModel.getModelName(), count));
        }
    ...
    private void handleManyToManyRecords(OColumn column, RelCommands command, OModel relModel,
        ...
        switch (command) {
            ...
            case Delete:
                ...
                // Deleting master record from relation modelName with given ids
    ...
    public void quickSyncRecords(ODomain domain)  {
        ...
        bundle.putString("sync_mode", Columns.SyncModel.Mode.QUICK_SYNC.name());
        syncAdapter.onPerformSync(getUser().getAccount(), bundle, authority(), null, new SyncResult());
    }
    ...
    public ODataRow quickCreateRecord(ODataRow record) {
        ...
        syncAdapter.setDomain(domain);
        Bundle bundle = new Bundle();
        bundle.putString("sync_mode", Columns.SyncModel.Mode.QUICK_CREATE.name());
        syncAdapter.onPerformSync(getUser().getAccount(), bundle, authority(), null, new SyncResult());
    ...
    public void isInstalledOnServer(final String module_name, IModuleInstallListener callback) {
        ...
        daoRepo.getOdoo(getUser()).installedOnServer(module_name, new IModuleInstallListener() {
            @Override
    ...
    public void onSyncStarted() {
        // Will be over ride by extending modelName
    }
    ...
    public DTO fromRow(ODataRow row, QueryFields queryFields) {
        return null;
    }

    public DTO get(int id, QueryFields queryFields){
        ODataRow row = browse(id);
        return fromRow(row, queryFields);
    }

    public  String makeFalseEmpty(String content) {
        return content.equals(Columns.FALSE) ? "" : content;
    }
