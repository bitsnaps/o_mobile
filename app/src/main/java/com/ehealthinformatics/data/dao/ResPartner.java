...
//            .setRelatedColumn("parent_id");
    ...
    public void initDaos(){
        stateDao = App.getDao(ResCountryState.class);
    }
    ...
    public Partner fromRow(final ODataRow row, QueryFields qf) {
        ...
        partner.setFullContact(getContact(App.getContext(), partner.getId()));
        return  partner;
    }
    ...
    public List<Partner> selectAll(CustomerList.Type type, QueryFields queryFields) {
        List<Partner>  partners
                = new ArrayList<>();
        for(ODataRow oDataRow: select()){
             partners.add( type == CustomerList.Type.Customer ? new Customer(fromRow(oDataRow, queryFields)) : fromRow(oDataRow, queryFields) );
        }
        return  partners;
    }
