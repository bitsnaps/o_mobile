 ...
 */
package com.ehealthinformatics.core.support;
...
import com.ehealthinformatics.core.rpc.helper.utils.OBundleUtils;
...
public class OUser {
    ...
    private String username, name, timezone, avatar, database, host, password, currencySymbol = OConstants.CURRENCY_SYMBOL;
    private Integer userId, partnerId, companyId, posSessionId = 0;
            //posJournalId = 0;
    ...
    public Bundle getAsBundle() {
        ...
        data.putString("currency_symbol", getCurrencySymbol());
        //End
    ...
    public void fillFromBundle(Bundle data) {
        ...
        if (OBundleUtils.hasKey(data, "currency_symbol"))
            setCurrencySymbol(data.getString("currency_symbol"));
    ...
    User userDTO = null;
    public User toDTO(ResUsers userDao) {
        userDTO = userDao.fromRow(userDao.browseServerId(getUserId()), QueryFields.all());
        return userDTO;
    }
    ...
    public void fillFromAccount(AccountManager accMgr, Account account) {
        ...
        setCurrencySymbol(accMgr.getUserData(account, "currency_symbol"));
